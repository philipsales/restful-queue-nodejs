...
function notificationAPI(body){
  ...
  logger.info("POST '%s'", `${process.env.EMAIL_URI}`);
  axios.post(`${process.env.EMAIL_URI}`, messages)
    .then((response) => {
      if (response.status == 200) {
        logger.info('EMAIL-SUBSCRIBER', 'sending alert was a success!');
      }

      logger.info('EMAIL-SUBSCRIBER ', `status code: ${response.status}`);
    }).catch(function(error){
      logger.error("EMAIL subscriber error response '%s'", error.message);
